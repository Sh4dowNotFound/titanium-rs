...
impl Shard {
    ...
    ///
    /// # Errors
    /// Returns `GatewayError::JsonDecode` if serialization fails, or `GatewayError::Closed` if the command channel is closed.
    pub fn send_payload<T: serde::Serialize>(&self, payload: &T) -> Result<(), GatewayError> {
        let json = self.serialize_payload(payload)?;
    ...
    /// Helper to serialize payload based on features
    #[allow(clippy::unused_self)]
    fn serialize_payload<T: serde::Serialize>(&self, payload: &T) -> Result<String, GatewayError> {
        #[cfg(feature = "simd")]
        {
            Ok(simd_json::to_string(payload)?)
        }
    ...
    ///
    /// # Errors
    /// Returns `GatewayError` if the connection fails or closes unexpectedly.
    ...
    async fn connect_and_run(
        ...
        *self.state.write() = ShardState::Connecting;
        ...
        let request = gateway_url.as_str().into_client_request()?;
        let host = request.uri().host().ok_or_else(|| GatewayError::Closed {
            code: 0,
            reason: "Invalid Gateway URL: missing host".to_string(),
        })?;
        ...
        *self.state.write() = ShardState::Handshaking;
        ...
        loop {
            ...
            tokio::select! {
                ...
                // Heartbeat timer
                () = sleep(next_heartbeat.saturating_duration_since(Instant::now())) => {
    ...
    fn build_gateway_url(&self) -> Result<Url, GatewayError> {
        ...
        let base_url = self
            ...
            .read()
            .as_ref()
            .map_or_else(|| self.config.gateway_url.clone(), |s| s.resume_url.clone());
    ...
    async fn handle_message(
        ...
        let action = match message {
            ...
            WsMessage::Close(frame) => {
                let (code, reason) = frame.map_or((0, String::new()), |f: CloseFrame| {
    ...
    fn process_frame(&self, text: &mut [u8]) -> Result<GatewayAction, GatewayError> {
        #[cfg(feature = "simd")]
        {
            ...
            let mut json = titanium_model::json::to_borrowed_value(text)
            ...
            // Parse OpCode
            // Clone is cheap here (Copy) but we can avoid it if we structure properly
            // Ideally we'd remove "op" from the map but that might be expensive if map is large
            // or if implementation doesn't support cheap remove.
            // For now, let's keep clone but use let-else for cleaner flow.
            // Optimize: Parse OpCode directly as u64 safely
            let op_num = json["op"].as_u64();
            let op = if let Some(n) = op_num {
                OpCode::from(u8::try_from(n).unwrap_or(0))
            } else {
                return Err(GatewayError::JsonDecode(
                    "Invalid opcode format".to_string(),
                ));
            };

            match op {
                OpCode::Dispatch => {
                    // Optimization: Remove 't' and 'd' to take ownership without cloning 'd'
                    // We must do this before borrowing 't' as string, to avoid conflict.

                    // Using let-else to flatten the nested match
                    let titanium_model::json::BorrowedValue::Object(ref mut map) = json else {
                        // Should technically be an error if it's not an object but we can ignore
                        return Ok(GatewayAction::None);
                    };

                    let Some(t_val) = map.remove("t") else {
                        return Ok(GatewayAction::None);
                    };
                    // If 'd' is missing, treat as null (though robust gateways usually send it)
                    let d = map
                        .remove("d")
                        .unwrap_or_else(|| titanium_model::json::BorrowedValue::from(()));

                    let Some(event_name) = t_val.as_str() else {
                        return Ok(GatewayAction::None);
                    };

                    let event_result = parse_event(event_name, d)?;
