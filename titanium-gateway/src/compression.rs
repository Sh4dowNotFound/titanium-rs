...
mod tests {
    ...
    fn test_zlib_stream_reuse() {
        ...
        let _msg1 = r#"{"op":10,"d":{"heartbeat_interval":41250}}"#;
        let _msg2 = r#"{"t":"READY","s":1,"op":0,"d":{"v":9}}"#;

        let mut encoder = ZlibEncoder::new(Vec::new(), Compression::default());
        let _decompressor = ZlibDecompressor::new();

        // Packet 1
        let _ = encoder.write_all(_msg1.as_bytes()); // Changed to use _msg1 and ignore result
        let _ = encoder.flush(); // Simulate sync flush, ignore result
        let _d1 = encoder.get_ref().clone(); // Get compressed data so far
